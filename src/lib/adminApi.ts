...
const mapStudentFromDb = (dbStudent: any, group?: GroupProfile, teacherName?: string): StudentProfile => ({
  ...
  paymentStatus: dbStudent.payment_status || 'unpaid',
...
export const adminApi = {
  ...
  async recordPayment(studentId: string, entry: Omit<PaymentHistoryEntry, 'id' | 'studentId'>): Promise<StudentProfile> {
    try {
      // Insert payment record
      const dbData = mapPaymentToDb(entry);
      const { data: payment, error: paymentError } = await supabase
        .from('payment_history')
        .insert({ ...dbData, student_id: studentId })
        .select()
        .single();
      ...
      const { data: student, error: studentError } = await supabase
        .from('students')
        .update({ payment_status: entry.status })
        .eq('id', studentId)
        .select()
        .single();
